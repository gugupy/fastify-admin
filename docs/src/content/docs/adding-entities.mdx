...
The show page can display related collections as separate tabs. By default the panel auto-detects array fields (e.g. `OneToMany` / `ManyToMany` relations) and renders a tab for each one. You can declare them explicitly with `relatedViews()` to control which relations appear and in what order:

```ts
export class UserView extends EntityView {
  relatedViews = ['posts', 'comments'];
}
```

Each entry must match the field name on the entity. The tab label is derived from the field name automatically. Omit `relatedViews()` entirely to fall back to auto-detection.
