...
- fastify-admin/
  - src/
    ...
    - cli/
      - index.ts  Unified CLI entry point (`npx fastify-admin <command>`)
...
### CLI (`npx fastify-admin <command>`)

All CLI commands are handled by `src/cli/index.ts`. Run them via `npx fastify-admin` from any project that has the package installed.

| Command | Description |
|---------|-------------|
| `init` | Scaffold a new project — prompts for a name and creates a directory with all starter files |
| `generate:resource <Name>` | Create `src/entities/<name>.entity.ts` and `src/views/<name>.view.ts` |
| `migrate:create <name>` | Create a new MikroORM migration file |
| `migrate:up` | Run all pending migrations |
| `migrate:down` | Roll back the last migration |
| `create-admin` | Interactive prompt to create a superuser account |
