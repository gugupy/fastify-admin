...
async function loadPermissions(em: EntityManager, userId: number): Promise<string[]> {
...
async function loginOrCreateOAuthUser(
    app: FastifyInstance,
    em: EntityManager,
    profile: { email: string; fullName: string; provider: string; oauthId: string },
    reply: any,
) {
    const fork = em.fork();
...
export async function registerAuthRoutes(
    app: FastifyInstance,
    em: EntityManager,
    opts: { signup: boolean; requireEmailVerification: boolean } = { signup: true, requireEmailVerification: false },
) {
    ...
    if (opts.signup) {
        app.post<{ Body: { fullName: string; username?: string; email: string; password: string } }>('/api/auth/signup', async (req, reply) => {
            const fork = em.fork();
        ...
        app.post<{ Body: { email: string; code: string } }>('/api/auth/signup/verify', async (req, reply) => {
            const fork = em.fork();
    ...
    app.post<{ Body: { userId: number; code: string } }>('/api/auth/mfa/verify', async (req, reply) => {
        const fork = em.fork();
    ...
    app.put<{ Body: { fullName: string; bio?: string } }>('/api/auth/profile', async (req, reply) => {
        try {
            await req.jwtVerify({ onlyCookie: true });
        } catch {
            return reply.status(401).send({ message: 'Unauthenticated.' });
    ...
    app.put<{ Body: { enabled: boolean } }>('/api/auth/mfa', async (req, reply) => {
        try {
            await req.jwtVerify({ onlyCookie: true });
        } catch {
            return reply.status(401).send({ message: 'Unauthenticated.' });
...
export async function registerOAuthRoutes(app: FastifyInstance, em: EntityManager, baseUrl: string) {
    ...
    if (process.env.GITHUB_CLIENT_ID && process.env.GITHUB_CLIENT_SECRET) {
        ...
        app.get('/api/auth/github/callback', async (req, reply) => {
            ...
            if (!primaryEmail) return reply.status(400).send({ message: 'No email found on GitHub account.' });
    ...
    if (process.env.MICROSOFT_CLIENT_ID && process.env.MICROSOFT_CLIENT_SECRET) {
        ...
        app.get('/api/auth/microsoft/callback', async (req, reply) => {
            ...
            const profile = await res.json() as { id: string; displayName: string; mail: string; userPrincipalName: string };
