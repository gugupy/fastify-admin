...
export async function seedRbac(em: EntityManager, registry: EntityRegistry) {
  ...
  let adminRole = await fork.findOne(
    Role,
    { name: 'Admin' },
  ...
  const adminPermNames = new Set(
    adminRole!.permissions.getItems().map((p) => p.name),
  )
  ...
  console.log(
    `[rbac] ${expectedNames.length} permissions ensured, Admin role synced.`,
  )
}
