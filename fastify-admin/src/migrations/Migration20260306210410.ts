...
export class Migration20260306210410 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table "permission_roles" ("permission_id" int not null, "role_id" int not null, constraint "permission_roles_pkey" primary key ("permission_id", "role_id"));`);

    this.addSql(`create table "user" ("id" serial primary key, "username" varchar(255) not null, "full_name" varchar(255) not null, "email" varchar(255) not null, "password" varchar(255) null, "oauth_provider" varchar(255) null, "oauth_id" varchar(255) null, "mfa_enabled" boolean not null default false, "mfa_code" varchar(255) null, "mfa_code_expires_at" timestamptz null, "bio" text not null default '');`);
    this.addSql(`alter table "user" add constraint "user_username_unique" unique ("username");`);

    this.addSql(`create table "role_users" ("role_id" int not null, "user_id" int not null, constraint "role_users_pkey" primary key ("role_id", "user_id"));`);
